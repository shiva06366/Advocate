...
if($result = mysqli_query($con,$sql))
{
  ...
  while($row = mysqli_fetch_assoc($result))
  {
	  ...
	  $todolist[$cr]['user_id'] = $row['user_id'];
	  for($i=1;$i<=143;$i++)
	  {
		  $access= "access".$i;
		$todolist[$cr][$access] = $row[$access];
	  }
