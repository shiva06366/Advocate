	  ...
	  while($row = mysqli_fetch_assoc($result))
		{
			...
			$currentDate=strtotime($request->data->date1);
			if($result1 =mysqli_query($con,$sql1))
			{
				...
				while($row2 = mysqli_fetch_assoc($result1))
				{
					...
					$currentDate += (86400);
						...
						for($Store;$Store<=$row2['date'];){
							if(strtotime($Store)==strtotime($row2['date'])){
							$todolist[$cr]['datas'][$cr1]['date'] = $row2['date'];
							$todolist[$cr]['datas'][$cr1]['time'] = $row2['time'];
							}
							else{
								$todolist[$cr]['datas'][$cr1]['date'] = $Store;
					$todolist[$cr]['datas'][$cr1]['time'] = "not av";
							}
							$currentDate += (86400);
							$Store = date('Y-m-d', $currentDate);
							//$cr1++;
