...
if($result = mysqli_query($con,$sql))
{
  ...
  while($row = mysqli_fetch_assoc($result))
  {
	...
	$sql_cou_cat= "SELECT `id`, `name` FROM `court_category` WHERE  id= '{$cou_cat}'";
   ...
   $todolist[$cr]['id']      = $row['id'];
    ...
    $todolist[$cr]['location']   = $row_loc['location'];
    $todolist[$cr]['court_category']   = $row_cou_cat['name'];
    $todolist[$cr]['court']   = $row_cou['name'];
    $todolist[$cr]['case_stage']   = $row_cas_st['name'];
